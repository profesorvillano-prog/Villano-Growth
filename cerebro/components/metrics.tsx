...
// Tableros de solo-lectura alimentados por las automatizaciones (Make / Meta / GHL / Hotmart):
//  · MetaLiveCard    — Meta Ads (espejo de cuenta o drill-down por anuncio)
//  · OrganicLiveCard — Instagram orgánico por pieza (reels/carruseles/posts/stories)
//  · HighTicketCard  — Embudo high ticket desde los pipelines de GoHighLevel
//  · VentasCard      — Compras y pagos reales (Hotmart / GHL)
// Cada consulta filtra por los slugs del cliente → cada cliente solo ve sus números.
...
export function MetaLiveCard({ slugs, color }: { slugs: string[]; color: string }) {
...
function MetaMirror({ slugs }: { slugs: string[] }) {
  ...
  if (rows.length === 0) return <Card className="p-5 text-sm text-dim">Sin datos de Meta para este período.</Card>;
  ...
  return (
    <Card>
      ...
      <p className="border-t border-line px-5 py-2.5 text-[11px] text-dim">
        Espejo de <span className="text-mute">Últimos 30 días</span> de Meta. Última sync: {syncedAt ? new Date(syncedAt).toLocaleString("es-CL") : "—"}.
      </p>
    </Card>
  );
}
...
function MetaDrilldown({ slugs, color }: { slugs: string[]; color: string }) {
  ...
  useEffect(() => {
    ...
    return () => { active = false; };
  }, [slugs.join(","), from, to]); // eslint-disable-line react-hooks/exhaustive-deps
...
export function VentasCard({ slugs }: { slugs: string[]; color?: string }) {
...
export function HighTicketCard({ slugs, color }: { slugs: string[]; color: string }) {
